...
class App extends Component {
  ...
  incrementCartItemQuantity = id => {
    this.setState(prevState => ({
      cartList: prevState.cartList.map(eachItem => {
        if (eachItem.id === id) {
          const newCount = eachItem.quantity + 1
          return {...eachItem, quantity: newCount}
        }
        return eachItem
      }),
    }))
  ...
  decrementCartItemQuantity = id => {
    ...
    const productCount = cartList.find(eachItem => eachItem.id === id)
      ...
      this.setState(prevState => ({
        cartList: prevState.cartList.map(eachItem => {
          if (productObject.id === eachItem.id) {
            const newCount = eachItem.quantity + product.quantity
